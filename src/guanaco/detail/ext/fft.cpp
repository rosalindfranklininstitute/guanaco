...
namespace guanaco {
  ...
  struct FFT<e_host>::Impl {
    ...
    Impl()
      : plan_forward(nullptr), plan_inverse(nullptr) {}
    ...
    void create_plan_2d(size_type xsize, size_type ysize) {
      ...
      plan_forward = fftwf_plan_dft_2d(ysize, xsize, data, data, FFTW_FORWARD, FFTW_ESTIMATE);
      plan_inverse = fftwf_plan_dft_2d(ysize, xsize, data, data, FFTW_BACKWARD, FFTW_ESTIMATE);
      fftw_free(data);
    }
  ...
  void FFT<e_host>::forward(void *data) const {
    ...
    fftwf_execute_dft(pimpl->plan_forward, v, v);
  }

  template <>
  void FFT<e_host>::inverse(void *data) const {
    fftwf_complex *v = reinterpret_cast<fftwf_complex *>(data);
    fftwf_execute_dft(pimpl->plan_inverse, v, v);
  }

  template <>
  FFT<e_host> FFT<e_host>::make_1d(size_type size) {
    FFT<e_host> self;
  ...
  template <>
  FFT<e_host> FFT<e_host>::make_2d(size_type xsize, size_type ysize) {
    FFT<e_host> self;
